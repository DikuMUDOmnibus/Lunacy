...
char *	const	where_name	[] = {
	"{G[{W Used as light     {G]{W:{x ",
    "{G[{C Worn on finger    {G]{W:{x ",
    "{G[{C Worn on finger    {G]{W:{x ",
    "{G[{C Worn around neck  {G]{W:{x ",
    "{G[{C Worn around neck  {G]{W:{x ",
    "{G[{C Worn on torso     {G]{W:{x ",
    "{G[{C Worn on head      {G]{W:{x ",
    "{G[{C Worn on legs      {G]{W:{x ",
    "{G[{C Worn on feet      {G]{W:{x ",
    "{G[{C Worn on hands     {G]{W:{x ",
    "{G[{C Worn on arms      {G]{W:{x ",
    "{G[{C Worn as shield    {G]{W:{x ",
    "{G[{C Worn about body   {G]{W:{x ",
    "{G[{C Worn about waist  {G]{W:{x ",
    "{G[{C Worn around wrist {G]{W:{x ",
    "{G[{C Worn around wrist {G]{W:{x ",
    "{G[{Y Wielded           {G]{W:{x ",
    "{G[{C Held              {G]{W:{x ",
};
...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
	...
	char level_s[MAX_STRING_LENGTH];

	if ( ! IS_OBJ_STAT(obj, ITEM_CORPSE_NPC )) 
		sprintf( level_s, "{G[{W%2d{G]{w ", obj->level );
    ...
    if ( IS_OBJ_STAT(obj, ITEM_GLOW)      )   strcat( buf, "{C(Glowing){w "   );
    if ( IS_OBJ_STAT(obj, ITEM_HUM)       )   strcat( buf, "{M(Humming){w "   );
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	if ( victim->position == POS_STANDING && victim->long_descr[0] != '\0' ) {
        ...
        sendf( ch, "{C[{W%2d{C] %s", victim->level, buf );
        return;
    }
...
void do_inspect( CHAR_DATA *ch, char *argument )
{
	...
	if ( mob ) {
		sendf( ch, "#BYou inspect [%s]{w\r\n", mob->short_descr       );
		sendf( ch, " Name: %s\r\n", mob->short_descr );
		sendf( ch, "Level: %d\r\n", mob->level       );
		return;
	}

	send_to_char( "There isn't anything like that here.\r\n", ch );
...
void do_exits( CHAR_DATA *ch, char *argument )
{
    ...
    strcpy( buf, fAuto ? "{B[Exits:" : "{gObvious exits:\r\n" );
...
/*
    sendf(ch, "{D+--------------------------------------------------------------------------+");
    sendf(ch, "|{w %38s                                   {D|",name_title);
    sendf(ch, "{D+-------------------+--------------+---------------------------------------+");
    sendf(ch, "| {B Race:{w %10s{D |{B STR: {r[{W%d{r/{w%d{r] {D|",
               race_table[ch->race].name, perm_stat[STAT_STR], get_curr_str(ch));
    sendf(ch, "| {BClass: {w%10s{D |{B INT: {r[{W%d{r/{w%d{r] {D|",
               profession_table[ch->profession].name, perm_stat[STAT_INT], get_curr_int(ch));
    sendf(ch, "| {BLevel: {w%10d{D |{B WIS: {r[{W%d{r/{w%d{r] {D|",
               ch->level, perm_stat[STAT_WIS], get_curr_wis(ch));
    sendf(ch, "| {B Rank: {w%10s{D |{B DEX: {r[{W%d{r/{w%d{r] {D|",
               ch->get_rank(), perm_stat[STAT_DEX], get_curr_dex(ch));
    sendf(ch, "| {B  Age: {w%10d{D |{B CON: {r[{W%d{r/{w%d{r] {D|",
               ch->get_age(), perm_stat[STAT_CON], get_curr_con(ch));
    sendf(ch, "{D+-------------------+--------------+---------------------------------------+");
*/
    char buf[MAX_STRING_LENGTH];
    AFFECT_DATA *paf;
