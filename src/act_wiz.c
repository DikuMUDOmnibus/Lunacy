...
void do_test( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp( argument, "battle" )) {
		call_lua ( ch->L, ch, NULL, LCALL_CHAR_SCRIPT, "do_battle", argument);
		return;
	}

	if (!str_cmp( argument, "map" )) {
		do_map ( ch, "border 2 world mobs text");
		return;
	}

	call_lua ( ch->L, ch, NULL, LCALL_CHAR_SCRIPT, "olcMedit", " ");

	if (ch->desc)
		ch->desc->alt_menu = AM_MEDIT;
...
void do_reset_char_lua( CHAR_DATA *ch, char *argument)
{
	if (ch->L) {
		lua_close(ch->L);
		load_char_lua(ch);
		sendf(ch, "#BReset Lua State for %s{w\r\n", ch->name);
	}
}
...
{
	extern int top_obj_index, top_mob_index, top_room;
	MOB_INDEX_DATA *mob;
	...
	if (arg1[0] == '\0') {
		send_to_char("Syntax: vnum <name>\r\n", ch);
		return;
	}

	if (arg1[1] == '\0' || arg2[1] == '\0') {
		send_to_char("vnum search must contain at least 2 characters\r\n", ch);
		return;
	}

	/* check the first argument for obj, mob, and room */
	if ( !str_cmp(arg1, "obj") ) {
		b_obj = TRUE;
	}
	...
	if ( str_cmp(arg1, "obj") || str_cmp(arg1, "mob") || str_cmp(arg1, "room") ) {
		b_obj  = TRUE;
		b_mob  = TRUE;
		b_room = TRUE;
	}

	sendf( ch, "Vnum Search for '%s'\r\n", argument );	


	/* objects */
	if (b_obj) {	
		send_to_char ("{BObjects:\r\n", ch);
		i = 0;

		for ( vnum = 0; i < top_obj_index; vnum++ ) {
			if ((obj = get_obj_index(vnum)) != NULL ) {
				i++;
				if ( is_name ( arg1, obj->name ) || is_name ( arg2, obj->name ) ) {
					find = TRUE;
					sendf( ch, "  {W[{C%5d{W] {w%s\r\n", vnum, obj->name);
				}
			}
		}
	...
	if (b_mob) {
		...
		for ( vnum = 0; i < top_mob_index; vnum++ ) {
			if ((mob = get_mob_index(vnum)) != NULL ) {
				i++;
				if ( is_name ( arg1, mob->short_descr ) || is_name ( arg2, mob->short_descr ) ) {
					find = TRUE;
					sendf( ch, "  {W[{C%5d{W] {w%s\r\n", vnum, mob->short_descr );
				}
			}
		}
	...
	if (b_room) {
		...
		for ( vnum = 0; i < top_room; vnum++ ) {
			if ((room = get_room_index(vnum)) != NULL ) {
				i++;
				if ( is_name( arg1, room->name ) || is_name ( arg2, room->name ) ) {
					find = TRUE;
					sendf( ch, "  {W[{C%5d{W] {w%s\r\n", vnum, room->name );
				}
			}
...
void do_medit( CHAR_DATA *ch, char *argument )
{
	...
	if ( ARG_NULL( argument ) ) {
		send_to_char ( "Synatax: medit <vnum>\r\n", ch );
		return;
	}

	if ( !vnum ) {
		send_to_char ( "Please provide a numeric vnum.\r\n", ch );
		return;
	}

	if ( (mob = get_mob_index( vnum )) != NULL ) {
	
		/* remove later: resets lua state for testing only */
		lua_close(ch->L);
		load_char_lua(ch);

		call_lua ( ch->L, ch, mob, LCALL_CHAR_SCRIPT, "olcMedit", " ");
