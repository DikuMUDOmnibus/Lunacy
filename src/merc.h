 ...
 * Lua Call types
 */
#define LCALL_CHAR_SCRIPT    1
#define LCALL_NUMBER         2
#define LCALL_COLOR          3
#define LCALL_OLC            4
...
struct	char_data {
    ...
    char *		long_descr;
    char *		description;
	char *      logon_str; /* just char* string to make it easier */
    short		sex;
    short		class;
    short		race;
    short		level;
    short		trust;
    short      ansi_color; // added for color
    int			played;
    time_t		logon;
    time_t		save_time;
    short		timer;
	...
	int 		huh; /* Huh? What? Used for non commands */
	lua_State*  L; /* added for Lua scripting */
};
...
/* external global lua state
 * defined in lua_interface.c
 */
extern lua_State *global_L;


/* alternate menus for input */
...
DECLARE_DO_FUN( do_reset_char_lua   );
...
void  sendf          args( ( CHAR_DATA *ch, char *fmt, ...) );
